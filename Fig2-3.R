...
# alldata$fit <- mydf$m[match(alldata$type1,mydf$sam)]
# alldata <- alldata %>% dplyr::filter(!is.na(fit))
head(alldata)

save(alldata,file="/mnt/data4/disk/chenfeng/HGT/exp/facs/data/merge_80.Rdata")
...
load("~/project/HGT/exp/facs/DPcaitaiG.Rdata")
alldata$caig <- DPcaitaiG$CAIg[match(alldata$Dp,DPcaitaiG$newDp)]
...
group.P %>% 
  ...
  style.print()
##fig.3c,e
fig.3ce <- group.P[,c(1,2,9)] %>% unique() 
fig.3ce$typeGmR <- as.numeric(as.vector(fig.3ce$typeGmR))
fig.3ce %>% ggplot(aes(x=typeGmR,y=mLpayoffratio))+
  geom_point()+
  geom_smooth(method = "lm",se=F)+
  scale_y_continuous(limits = c(1,3.5),breaks = c(1,2,3))+
  style.print()
fig.3ce %>% ggplot(aes(x=typeGmR,y=mLcostratio))+
  geom_point()+
  geom_smooth(method = "lm",se=F)+
...
cor.test(fig.3ce$typeGmR,fig.3ce$mLpayoffratio,method = "s")
cor.test(fig.3ce$typeGmR,fig.3ce$mLcostratio,method = "s")
...
###figs1ab
load("/mnt/data4/disk/chenfeng/HGT/exp/facs/data/draw.expression3.Rdata")
tmpdd <- cor.test(alldata$Dp,alldata$payoff,method = "s")
as.vector(tmpdd$p.value)
...
fit <- lapply(1:length(unique(mydf$type)),function(x){
    ...
    if(as.numeric(cor.test(b$dp,b$nn,method = metmpdd)$estimate[1])>0){
      res2 <- data.frame(rho=as.vector(cor.test(b$dp,b$nn,method = metmpdd )$estimate[1]),
                         ...
                         stringsAsFactors = F,GmCon=unique(mydf$type)[x],type="fit")
...
figs1$type <- factor(figs1$type,levels = c("fit","cost","payoff"))
